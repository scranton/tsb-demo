...
delete_gce_lb_objects() {
  ...
  set +e
  gcloud compute --project="${PROJECT}" --quiet firewall-rules   delete "k8s-fw-${id}"
  gcloud compute --project="${PROJECT}" --quiet forwarding-rules delete "${id}"        --region="${REGION}"
  gcloud compute --project="${PROJECT}" --quiet target-pools     delete "${id}"        --region="${REGION}"
  set -e
}

check_firewalls() {
  # ACTIVE_IPS=$(kubectl get services --context='local-cluster' --all-namespaces -o json | jq -r '.items[].status.loadBalancer.ingress[0].ip' | sort | uniq)
...
check_target_pools() {
  local delete=()
  local current_nodes current_forwarding_rules target_json target_name
